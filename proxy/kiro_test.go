...
// These three historical normalization regressions now exercise the stream
// parser directly. Kiro emits incremental deltas, so prefix- and overlap-shaped
// chunks must be forwarded verbatim rather than treated as cumulative snapshots.
func TestNormalizeChunkBasicProgression(t *testing.T) {
	got := parseAssistantChunksForTest(t, "abc", "abcde")
	if got != "abcabcde" {
		t.Fatalf("expected incremental chunks to pass through, got %q", got)
	}
}

func TestNormalizeChunkPrefixRewindDoesNotReplay(t *testing.T) {
	got := parseAssistantChunksForTest(t, "abcde", "abc", "abcdef")
	if got != "abcdeabcabcdef" {
		t.Fatalf("expected prefix-shaped deltas to pass through verbatim, got %q", got)
	}
}

func TestNormalizeChunkOverlapDelta(t *testing.T) {
	got := parseAssistantChunksForTest(t, "hello world", "world!!!")
	if got != "hello worldworld!!!" {
		t.Fatalf("expected overlapping deltas to pass through verbatim, got %q", got)
	}
}

func parseAssistantChunksForTest(t *testing.T, chunks ...string) string {
	t.Helper()
...
func TestBuildKiroTransportFallsBackToEnvironmentProxy(t *testing.T) {
	...
	// http.ProxyFromEnvironment caches the process environment on first use, so
	// mutating HTTPS_PROXY here is inherently order-dependent in a full suite.
	// Verify that the transport installs the standard resolver without invoking
	// it; the standard library owns its environment parsing behavior.
...
func TestInitKiroHttpClientKeepsShortRestTimeout(t *testing.T) {
	...
	if streamClient.Timeout != 30*time.Minute {
		t.Fatalf("expected streaming timeout to be 30m, got %s", streamClient.Timeout)
	}
...
// TestCallKiroAPIRebuildsRegionForApiKeyAccount preserves the v3 region
// regression check while reflecting the merged CLI runtime endpoint.
func TestCallKiroAPIRebuildsRegionForApiKeyAccount(t *testing.T) {
	...
	kiroHttpStore.Store(&http.Client{
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			return &http.Response{
				...
				Body: io.NopCloser(bytes.NewReader(awsEventStreamFrame(t,
					"assistantResponseEvent", map[string]interface{}{"content": "ok"}))),
	...
	apiKeyAccount := &config.Account{
		AuthMethod:  "api_key",
		KiroApiKey:  "k",
		AccessToken: "k",
		ApiRegion:   "eu-central-1",
	}
	if err := CallKiroAPI(apiKeyAccount, newKiroRetryTestPayload(), nil); err != nil {
		t.Fatalf("CallKiroAPI: %v", err)
	}
	if gotHost != "runtime.eu-central-1.kiro.dev" {
		t.Fatalf("API-key host: want runtime.eu-central-1.kiro.dev, got %q", gotHost)
	}
}

// TestAccountEmailForLogMasksApiKey ensures direct-key accounts remain
// identifiable in logs without exposing the whole key.
func TestAccountEmailForLogMasksApiKey(t *testing.T) {
	account := &config.Account{AuthMethod: "api_key", KiroApiKey: "ksk_abcdefghijklmnop"}
	if got := accountEmailForLog(account); got != "ksk_ab…mnop" {
		t.Fatalf("API-key fallback: want %q, got %q", "ksk_ab…mnop", got)
	}
	account.Email = "user@example.com"
	if got := accountEmailForLog(account); got != "user@example.com" {
