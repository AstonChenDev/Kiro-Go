...
// a transient non-auth upstream failure whose body happens to contain "expired"
// must not disable an otherwise healthy account.
func TestRefreshAccountInfoDoesNotBanOnTransientUpstreamError(t *testing.T) {
	...
	requestAccount := account
	if _, err := RefreshAccountInfo(&requestAccount); err == nil {
		t.Fatal("expected GetUsageLimits 500 error to propagate")
	}
	accounts := config.GetAccounts()
	if len(accounts) != 1 {
		t.Fatalf("expected one account, got %d", len(accounts))
	...
	if !accounts[0].Enabled || accounts[0].BanStatus == "BANNED" {
		t.Fatalf("transient 5xx must not ban the account: enabled=%v status=%q reason=%q",
			accounts[0].Enabled, accounts[0].BanStatus, accounts[0].BanReason)
	}
}

// TestRefreshAccountInfoDoesNotBanApiKeyOnUsageLimit403 pins the invariant that
// a usage refresh cannot authoritatively ban a direct API-key credential.
func TestRefreshAccountInfoDoesNotBanApiKeyOnUsageLimit403(t *testing.T) {
	...
	requestAccount := account
	if _, err := RefreshAccountInfo(&requestAccount); err == nil {
		t.Fatal("expected GetUsageLimits 403 to propagate")
	}
	accounts := config.GetAccounts()
	if len(accounts) != 1 {
		t.Fatalf("expected one account, got %d", len(accounts))
	...
	if !accounts[0].Enabled || accounts[0].BanStatus == "BANNED" {
		t.Fatalf("API-key account must not be banned on usage-limit 403: enabled=%v status=%q reason=%q",
			accounts[0].Enabled, accounts[0].BanStatus, accounts[0].BanReason)
	}
}
