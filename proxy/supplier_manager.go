...
func (m *supplierManager) executeIntent(intent supplierPurchaseIntent) (supplierPurchaseOutcome, error) {
	...
	response, err := m.apiFactory(*provider).Purchase(intent.Count, intent.Region, intent.ClientOrderID)
	if err != nil {
...
func (m *supplierManager) processPendingIntents() {
	...
	for _, intent := range m.store.pendingIntents() {
		if intent.Attempts > 0 {
			...
			if supplierNow().Unix()-intent.UpdatedAt < int64(backoff/time.Second) {
				continue
			}
		}
