...
func (h *Handler) performWebSearch(model, query string) (*WebSearchResults, string, *config.Account, error) {
	...
	for attempt := 0; attempt < retryBudget; attempt++ {
		...
		if !h.pool.Acquire(account.ID, false) {
			// Selection itself reserves the smart-scheduler in-flight slot.
			// Acquire rejection is neutral account health, but must still finish
			// that selection before trying another account.
		...
		if err := h.ensureValidToken(account); err != nil {
			h.pool.Release(account.ID, false)
			lastErr = err
			excluded[account.ID] = true
