...
func (h *Handler) callUpstreamForWebSearch(req *ClaudeRequest, thinking bool, estimatedInputTokens int) (*webSearchRoundOutcome, *config.Account, error) {
	...
	for attempt := 0; attempt < retryBudget; attempt++ {
		...
		if !h.pool.Acquire(account.ID, req.Stream) {
			// GetNextForModelExcluding already marked this account selected.
			// Finish that scheduler slot neutrally when the RPM/SSE gate rejects
			// the acquisition, then continue with a different account.
		...
		if err := h.ensureValidToken(account); err != nil {
			h.pool.Release(account.ID, req.Stream)
			lastErr = err
			excluded[account.ID] = true
		...
		// Each agentic round is a complete upstream dispatch. Finish and release
		// it here instead of holding all intermediate rounds until the terminal
		// response.
