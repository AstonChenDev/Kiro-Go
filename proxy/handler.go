...
// normalizeImportedMachineID accepts the two device-id formats already used by
// this ecosystem: UUIDs generated by Kiro-Go and 64-hex SHA-256 identifiers
// exported by kiro-login-web. Rejecting arbitrary text keeps the value safe for
// the User-Agent/header path where it is later used.
func normalizeImportedMachineID(value string) (string, bool) {
	value = strings.TrimSpace(value)
	if value == "" {
		return "", true
	}
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// 路由
	switch {
	// Service-to-service credential ingestion. This is deliberately outside the
	// admin API: it has a separate, import-only token and cannot access any other
	// management operation.
	case path == "/internal/v1/credentials/import" || path == "/internal/v1/credentials/status":
		h.handleCredentialIntegration(w, r)
...
func (h *Handler) apiImportCredentials(w http.ResponseWriter, r *http.Request) {
	...
	importedMachineID, validMachineID := normalizeImportedMachineID(req.MachineID)
	if !validMachineID {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": "machineId must be a UUID or 64 hexadecimal characters"})
		return
	}
