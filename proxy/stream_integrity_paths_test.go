...
func setupIntegrityPathTest(t *testing.T, server *httptest.Server) *Handler {
	...
	// The pool is process-global and retains RPM/in-flight bookkeeping across
	// tests that reuse "test-account". Clear it before reloading this isolated
	// config so a previous path cannot make Acquire reject before dispatch.
...
func TestOpenAIStreamEmitsErrorOnTruncatedStream(t *testing.T) {
	...
	if active := h.pool.GetActiveSSE("test-account"); active != 0 {
		t.Fatalf("truncated stream leaked SSE slot: active=%d", active)
	}
	if stats := h.pool.GetRuntimeStatsSnapshot()["test-account"]; stats.InFlight != 0 {
		t.Fatalf("truncated stream leaked scheduler in-flight slot: %+v", stats)
	}
}
