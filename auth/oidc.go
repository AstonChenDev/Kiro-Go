...
func normalizeAWSRegionOrDefault(region string) (string, error) {
	if strings.TrimSpace(region) == "" {
		region = "us-east-1"
	}
	return config.NormalizeAWSRegion(region)
}
...
func refreshOIDCToken(refreshToken, clientID, clientSecret, region string, client *http.Client) (string, string, int64, string, error) {
	...
	if err != nil {
		return "", "", 0, "", fmt.Errorf("OIDC refresh region rejected: %w", err)
	}
