...
// validateMicrosoftExternalIdpEndpoint is the strict production validator behind
// ValidateExternalIdpEndpoint. Keeping the exported wrapper swappable preserves
// the existing httptest seam without weakening the default trust boundary.
func validateMicrosoftExternalIdpEndpoint(rawURL string) error {
...
func refreshExternalIdpToken(refreshToken, clientID, tokenEndpoint, issuerURL, scopes string, client *http.Client) (string, string, int64, string, error) {
	...
	} else if err := ValidateExternalIdpEndpoint(tokenEndpoint); err != nil {
		// Compatibility for existing v3 external_idp records that predate
		// issuerUrl persistence. The endpoint still crosses the strict outbound
		// allow-list boundary; all newly created Microsoft records are tenant-bound.
		return "", "", 0, "", err
	}
	form := url.Values{}
...
func DeriveExternalIdpEndpoints(userID, clientID, accessToken string) (tokenEndpoint, issuerURL, scopes string) {
	...
	if issuerURL == "" {
		...
		if normalized := issuerFromKAMUserID(issuerURL); normalized != "" {
